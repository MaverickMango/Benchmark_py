...
patterns = {
    ...
    'total_time': re.compile(r"Total time:\s+([\d.]+)\s+seconds"),
...
def parse_log(file_path):
    ...
    result = {
        ...
        'tests_failures': None,
        'tests_errors': None,
        'test_time_elapsed_seconds': None
    }
    if not os.path.exists(file_path):
        return result
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in reversed(lines):
                if count == 3:
                    break
                if patterns['build_status'].search(line):
                    count += 1
                    result['build_status'] = True
                if match := patterns['total_time'].search(line):
                    count += 1
                    result['total_time_seconds'] = float(match.group(1))
                if match := patterns['junit_result'].search(line):
                    count += 1
